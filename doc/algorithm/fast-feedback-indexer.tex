...
\For{$j=1\, ...\, 4$}
  ...
  \If{$trace(\mat{F}) < m$}\label{alg:vecrefinement:nclose}\Comment{too few \emph{close spots}?}
    \State exit for loop
  \EndIf
...
The set $\mathcal{P}_i^{refined}$ can be computed in parallel, working on all valid $(i,p)$ pairs.
...
This axis is unique, except if $\vect{a}_i$ and $\vect{a}_{i,p}^{refined}$ are colinear, in which case any axis perpendicular to $\vect{a}_i$ will serve the purpose. The collinearity check is done with a small parameter $\tau$, as shown in line \ref{alg:mirroraxis:collinear} of Algorithm \ref{alg:mirroraxis}. As vector $\vect{w}$ in Algorithm \ref{alg:mirroraxis} another original unit cell vector, like $\vect{a}_{i+}$ will do fine to construct a perpendicular axis.
...
The mirroring operation that brings row vector $\vect{x}$ to $\vect{y}$ by mirroring on row vector $\vect{r}_p$ is implemented in Algorithm \ref{alg:mirror}.
...
First $\vect{u}_x$ is constructed using $\vect{a}_{i+,p}$ projected to the plane normal to $\vect{u}_z$, yielding the vector $\vect{u}_x$ after normalization. The vector $\vect{u}_y$ then simply is $-\vect{u}_x\times \vect{u}_z$, see Algorithm \ref{alg:basis}.

\begin{algorithm}
\caption{Basis construction}
\label{alg:basis}
\begin{algorithmic}[1]
\Function{basis}{$\vect{a}_{i,p}^{refined},\vect{a}_{i+,p}$}\Comment{all row vectors}
\State $\vect{u}_z\gets \frac{\vect{a}_{i,p}^{refined}}{\|\vect{a}_{i,p}^{refined}\|}$
\State $\vect{t}\gets \vect{a}_{i+,p} - \vect{u}_z\vect{a}_{i+,p}^T \vect{a}_{i,p}^{refined}$
...
\label{alg:rotate}
\begin{algorithmic}[1]
\Function{rotate}{$\vect{x},\vect{u}_x,\vect{u}_y,\vect{u}_z,\alpha_q$}\Comment{all row vectors}
\State $z\gets \vect{x}\vect{u}_z^T$, $x\gets \vect{x}\vect{u}_x^T$, $y\gets \vect{x}\vect{u}_y^T$
...
\begin{algorithm}
\caption{Cell orientation sampling point}
...
\State $\alpha\gets \frac{2\pi q}{Q^*}$
\State $\vect{a}_{i+,p,q}\gets \text{rotate}(\vect{a}_{i+,p}, \vect{u}_x, \vect{u}_y, \vect{u}_z, \alpha)$
\State $\vect{a}_{{i+}+,p,q}\gets \text{rotate}(\vect{a}_{{i+}+,p}, \vect{u}_x, \vect{u}_y, \vect{u}_z, \alpha)$
...
Similar to the construction of $P$ candidate vectors per unit cell vector, $\mathcal{P}_i$, candidate cells are selected based on a scoring function for cells, shown in Algorithm \ref{alg:cellscore}. The scoring is similar to Algorithm \ref{alg:pointscore} for vector orientations.
...
\begin{algorithmic}[1]
\Function{vector-refinement}{$\mat{B}_{i,p,q},\mat{S},score_{i,p,q},maxiter,m,\lambda$}
...
\State $t = \floor{score_{i,p,q}} + 1 - score_{i,p,q}$\Comment{mean distance from $score$}
\For{$j=1\, ...\, maxiter$}
  \State $\mat{F}\gets \filter{\distvecZ{\mat{S}\vect{B}^T} < t}$\Comment{filter for \emph{close spots}}
  \If{$trace(\mat{F}) < m$}\label{alg:vecrefinement:nclose}\Comment{too few \emph{close spots}?}
    \State exit for loop
  \EndIf
  \State $\mat{B}\gets \argmin_{\mat{X}} \| \mat{F}(\mat{S}\mat{X}^T - \mat{C}_{i,p,q}) \|^2$\label{alg:vecrefinement:lsq}\Comment{least squares on \emph{close spots}}
...
Applying the refinement in Algorithm \ref{alg:cellrefinement} to all cell orientation candidates in the set $\mathcal{Q}$, we end up with the final set of unit cell candidates

\[
 \mathcal{Q}^{refined} = \{ \mat{B}_{i,p,q}^{refined} \mid (i,p,q)\in \mathcal{Q} \}
\]

\subsection{Approximative Solution}
