...
$-| \langle s \in spots: dist(s, clp) < h \rangle | + 2^{\frac{\sum_{s \in spots} \log_2(trim_l^h(dist(s, clp)) + delta))}{|spots|}} - delta$, where $trim$ stands for trimming, $dist(s, clp)$ for the distance of a spot to the closest lattice point, and $l,h$ are the lower and higher trimming thresholds.
...
*'ifss'*: Iteratively fit a new cell to $\langle s \in spots: dist(s, clp) < t \rangle$ and contract the threshold. Stop when the number of spots is below the minimum number of spots.

*'ifse'*: Iteratively fit an additive delta to the errors $\langle dist(s, clp) : s \in spots \land dist(s, clp) < t \rangle$ and contract the threshold. Stop when the maximum number of iterations is reached, or the errors set size is below the minimum number of spots.
